...
// 콘텐츠는 임의 창작 금지. 메타(sector/division/platform)는 content/company.md 조직 정보와
// defense-tech 관례에 따라 부여 — Log.md [DECISION] 참조.
...
export interface CaseItem {
  ...
  slug: string;
  title: string;
  cat: string; // 카드 카테고리 라벨 (eyebrow)
  badge: string; // 미디어 위 배지
  desc: string; // 카드 1줄 요약
  image: string; // /images/*.jpg (대표)
  figures: string[]; // 상세 보조 이미지 (1~2)
  cats: CaseCategory[]; // 필터용
  featured?: boolean;
  // 상세
  sector: string;
  division: string;
...
// 필터 칩 순서
export const FILTERS: { cat: "all" | CaseCategory; label: string }[] = [
  { cat: "all", label: "전체" },
  { cat: "defense", label: "국방·항공" },
  { cat: "hils", label: "HILS·SIL" },
  { cat: "propulsion", label: "추진·시험" },
  { cat: "nuclear", label: "원자력·에너지" },
  { cat: "engineering", label: "엔지니어링" },
];
...
// 연관 사례: 같은 카테고리 우선, 자기 자신 제외, 최대 3개
